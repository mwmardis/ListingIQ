...
### Task 3: Enhance Redfin Scraper to Extract New Fields

**Files:**
- Modify: `listingiq/scrapers/redfin.py:175-200` (_parse_listings)
- Test: `tests/test_scrapers.py` (new file or add to existing)
...
# Minimal Redfin API response for a single home
_SAMPLE_HOME = {
    "price": {"value": 250000},
    "beds": 3,
    "baths": 2,
...
class TestRedfinFieldExtraction:
    def test_extracts_stories(self):
        ...
        scraper = RedfinScraper(ScraperConfig())
...
Expected: FAIL — scraper doesn't extract stories/pool yet

**Step 3: Write minimal implementation**

In `listingiq/scrapers/redfin.py`, in `_parse_listings`, add extraction inside the `Listing(...)` constructor (around line 175-200):
...
Note: Redfin's stingray API response format varies — `stories` and `pool` may be top-level or nested. Extract what's available, default to 0/None. Store any extra data in `raw_data` for future use.
...
```bash
git add listingiq/scrapers/redfin.py tests/test_scrapers.py
git commit -m "feat: extract pool and stories from Redfin scraper"
```
